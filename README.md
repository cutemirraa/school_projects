# School Projects Portfolio

## Python Projects

### 1. tDCS for Allergic Rhinitis
- **Folder:** `rci_tdcs_analysis`
- **Description:** Statistical analysis of a fictional RCT evaluating tDCS effectiveness
- **Key tools:** Python, pandas, seaborn, statistical tests
- [View project](./rci_tdcs_analysis/README.md)

### 2. Video Games Data Preprocessing
- **Folder:** `video_games_preprocessing`
- **Description:** Preprocessing and cleaning of historical video game sales data (2000–2013)
- **Key tools:** Python, pandas
- [View project](./video_games_preprocessing/README.md)

### 3. Moscow Catering Market Analysis
- **Folder:** `moscow_catering_analysis`
- **Description:** Exploratory data analysis of Moscow's public catering market (summer 2022)
- **Key tools:** Python, pandas, seaborn, matplotlib, phik
- [View project](./moscow_catering_analysis/README.md)

## SQL & Visualization Projects

### 4. Real Estate Market Analysis (SPb & Leningrad Oblast)
- **Folder:** `real_estate_analysis`
- **Description:** Analysis of property listings to identify attractive segments and seasonality trends
- **Key tools:** SQL, Yandex DataLens
- [View project](./real_estate_analysis/README.md)

### 5. Donor Activity Analysis
- **Folder:** `donor_analysis`
- **Description:** Analysis of donor engagement, bonus impact, social networks, and donation planning vs reality
- **Key tools:** SQL
- [View project](./donor_analysis/README.md)

---

## About
This repository contains my data analysis projects created for educational and portfolio purposes.

## Author
Zaripova Almira
